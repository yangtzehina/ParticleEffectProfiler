...
namespace ParticleEffectProfiler.Runtime
{
    /// <summary>
    /// 处理特效整体相关的数据
    /// </summary>
    public class GetParticleEffectData
    {
        ...
        public static string GetGetRuntimeMemorySizeStr(GameObject go)
        {
            ...
            return string.Format("贴图所占用的内存：{0}   建议：<{1}\n贴图数量：{2}     建议：<{3}", memorySizeStr, maxMemorySizeStr,
        ...
        {
            int max = 10;
            return string.Format("DrawCall: {0}   最高：{1}   建议：<{2}",
                FormatColorMax(GetOnlyParticleEffecDrawCall(), max), FormatColorMax(m_MaxDrawCall, max), max);
        }
        ...
        {
            int max = 50;
            return string.Format("粒子数量：{0}   最高：{1}   建议：<{2}",
                FormatColorMax(particleEffectGo.GetParticleCount(), max),
                FormatColorMax(particleEffectGo.GetMaxParticleCount(), max), max);
        }
